...
/* ══════════════════════════════════════════════════════════
   JURATONE PRO v8.6.3 — Edition Expert (Belgique)
   Patch applique : separation claire bloque / calcul specifique
   dans le module d'indexation. Code nettoyé, sans emoji.
   ══════════════════════════════════════════════════════════ */
...
const LEGIS = [
  ...
  { id: "l4", title: "Reforme du Code civil", desc: "Nouveaux Livres en vigueur (SPF Justice).", url: "https://justice.belgium.be/fr/themes_et_dossiers/droit_civil/nouveau_code_civil", cat: "Code" },
...
const GUIDE_SECTIONS = [
  { id: "g-juris", title: "Jurisprudence", category: "juridique", intro: "La section Jurisprudence regroupe les liens directs vers les bases de donnees officielles de jurisprudence belge et europeenne.", steps: ["Identifiez la juridiction concernee.", "Cliquez sur la source correspondante. Juportal est le moteur principal gere par le SPF Justice.", "Utilisez les mots-cles : numero d'article, nom des parties, date.", "Pour les droits fondamentaux, consultez HUDOC (CEDH) en parallele."], tips: ["Commencez par Juportal pour la jurisprudence publique.", "Les arrets de la Cour constitutionnelle sont numerotes par annee."], pitfalls: ["Ne confondez pas un arret de cassation avec un jugement de premiere instance.", "La jurisprudence belge n'est pas un systeme de precedent obligatoire."] },
  ...
  { id: "g-legis", title: "Legislation", category: "juridique", intro: "Liens vers les textes legaux officiels.", steps: ["Commencez par Justel (legislation consolidee).", "Consultez le Moniteur belge pour les modifications recentes.", "EUR-Lex pour le droit europeen.", "Vlaamse Codex ou Wallex pour le regional."], tips: ["Justel affiche les textes consolides a jour.", "SenLex est utile pour la Constitution."], pitfalls: ["Ne citez jamais un article sans verifier la version consolidee.", "Pas de hierarchie entre loi federale et decret."] },
  ...
  { id: "g-succession", title: "Droits de succession", category: "outils", intro: "Estimation des droits de succession par region et lien de parente.", steps: ["Entrez le montant herite net.", "Selectionnez la region du dernier domicile du defunt.", "Choisissez le lien de parente.", "Cliquez sur Calculer."], tips: ["Les droits s'appliquent par tranche progressive.", "Le logement familial peut etre exonere sous conditions regionales.", "Declaration : 4 mois (Belgique), 5 mois (Europe), 6 mois (hors Europe)."], pitfalls: ["Le calcul ne tient pas compte des assurances-vie.", "Le calcul ne vaut que si la valeur saisie correspond exclusivement a la part nette concernee par le regime applicable."] },
  ...
  { id: "g-indexation", title: "Module d'indexation", category: "outils", intro: "Calcul d'indexation base sur l'Indice Sante. Jeu de donnees local de demonstration.", steps: ["Chargez le jeu de donnees local.", "Entrez le montant de base.", "Selectionnez la region et le PEB pour les verifications de blocage.", "Cliquez sur Calculer."], tips: ["Pour les baux, l'indexation ne peut se faire qu'une fois par an.", "A Bruxelles, certaines indexations supposent un bail enregistre et un certificat PEB valide communique. En Wallonie, le calcul peut devenir specifique selon le PEB et la date pertinente du bail."], pitfalls: ["Les donnees affichees sont un echantillon de demonstration.", "En Flandre post-2019, l'indice de base est le mois precedant l'entree en vigueur."] },
];
...
const calcReducedHomeTax = (amount, reg) => {
  if (reg === "wallonie") {
    return amount <= 20000 ? amount * 0.01 : 20000 * 0.01 + (amount - 20000) * 0.02;
  }
  if (reg === "bruxelles") {
    return amount <= 50000 ? amount * 0.02 : 50000 * 0.02 + (amount - 50000) * 0.05;
  }
  return null;
};
...
export default function JuratonePro() {
  ...
  const loadDemoData = () => { setIdxApiState("loading"); const realResponse = [{ month: "Mars 2024", value: 131.62 }, { month: "Avril 2024", value: 132.09 }, { month: "Mai 2024", value: 132.32 }, { month: "Juin 2024", value: 132.55 }, { month: "Juillet 2024", value: 133.00 }, { month: "Aout 2024", value: 133.40 }, { month: "Septembre 2024", value: 133.05 }, { month: "Octobre 2024", value: 133.56 }, { month: "Novembre 2024", value: 133.91 }, { month: "Decembre 2024", value: 133.73 }, { month: "Janvier 2025", value: 135.52 }, { month: "Fevrier 2025", value: 135.79 }]; setIdxData(realResponse); setIdxBaseMonth(realResponse[0].month); setIdxNewMonth(realResponse[realResponse.length - 1].month); setIdxApiState("success"); };
  
  // PATCH v8.6.3 : doIndexation mis a jour avec blocked / needsOfficialCalculator
  const doIndexation = () => { 
  ...
  const doSucc = () => { 
    ...
    const applyReducedRateToHome = () => { advantageMode = "reduced_rate"; };
    ...
    if (sR === "flandre") {
      ...
      if (sFamWallonie5ans) { applyExemptionToHome(); } else { warning = "Wallonie : necessite 5 ans de residence principale."; }
    } 
      ...
      const reduced = calcReducedHomeTax(home, "bruxelles");
      if (reduced !== null) { taxHome = reduced; advantageMode = "reduced_rate"; }
    }
      ...
      if (sRel === "Conjoint / Cohabitant legal" && !sFamBruxellesExclu) { applyExemptionToHome(); }
        ...
        if (sBruxCohab3yr) {
          warning = "Bruxelles : cohabitation de fait effective + menage commun >= 3 ans. Un tarif reduit sur le logement familial est possible, mais le montant exact depend du regime legal applicable.";
          advantageMode = "reduced_rate";
          advantageNotCalculated = true;
        } 
        else if (sBruxCohab1yr) {
          warning = "Bruxelles : cohabitation de fait effective + menage commun >= 1 an. Tarif partenaire applicable (sans exoneration du logement familial).";
  ...
  return (
    <div style={{ fontFamily: "'Libre Baskerville',Georgia,serif", background: T.bg, color: T.txt, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 24px 60px" }}>
        ...
        {pg === "indexation" && (<div className="no-print"><SectionHead title="Indexation (Loyer & Contrats)" sub="Calcul d'indexation base sur l'Indice Sante. Jeu de donnees local de demonstration 2024-2025." />
          <Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontSize: 16, fontWeight: 700 }}>Jeu de donnees local de demonstration 2024-2025</div><div style={{ fontSize: 13, color: T.muted, marginTop: 4 }}>Indices Sante de reference (Base 2013). Non connecte a l'API Statbel.</div></div><button onClick={loadDemoData} disabled={idxApiState === "loading"} style={{ padding: "10px 18px", background: idxApiState === "loading" ? T.dim : T.priGrad, border: "none", borderRadius: 8, color: "white", fontFamily: "'Libre Franklin',sans-serif", fontSize: 13, fontWeight: 700, cursor: idxApiState === "loading" ? "wait" : "pointer" }}>{idxApiState === "loading" ? "Chargement..." : idxApiState === "success" ? "Donnees chargees" : "Charger les indices (demo)"}</button></div>
          ...
          )}<Disclaimer>Pour la Wallonie et Bruxelles (PEB defavorables), privilegiez le calculateur officiel SPF/Statbel.</Disclaimer><SourceBadge type="Source" source="Echantillon de demonstration 2024-2025" bareme="Base 2013" /><PrintBtn /></Card>)}
        </div>)}

        {pg === "jurisprudence" && (<div className="no-print"><SectionHead title="Jurisprudence" sub="Bases de donnees officielles." />{JURIS.map(j => (<Card key={j.id}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}><div><div style={{ fontSize: 16, fontWeight: 600, marginBottom: 2 }}>{j.title}</div><div style={{ fontSize: 13, color: T.muted, fontFamily: "'Libre Franklin',sans-serif" }}>{j.desc}</div></div>{j.url && <LinkBtn href={j.url}>Ouvrir</LinkBtn>}</div></Card>))}</div>)}
        ...
        {pg === "notariat" && (<div><SectionHead title="Outils notariaux" sub="Calculateurs et dossiers notaries." />
          ...
          {notSub === "immo" && (<div><Card className="no-print"><Divider label="Calculateur de frais d'achat" color={T.pri} /><div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {iR === "flandre" && (<div style={{ background: T.priL, padding: 12, borderRadius: 8, border: `1px solid ${T.pri}44`, marginTop: 8 }}><div style={{fontSize:11, fontWeight:700, color:T.pri, marginBottom:8}}>CONDITIONS FLANDRE 2026 (Taux 2%)</div>
              <label onClick={()=>{setIFlandreNatural(!iFlandreNatural);setIRes(null);}} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,cursor:"pointer",fontFamily:"'Libre Franklin',sans-serif",fontSize:12}}><div style={{width:16,height:16,borderRadius:4,background:iFlandreNatural?T.cyan:T.s3,border:`1px solid ${iFlandreNatural?T.cyan:T.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10}}>✓</div>Personnes physiques uniquement</label>
              <label onClick={()=>{setIFlandreFullOwnership(!iFlandreFullOwnership);setIRes(null);}} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,cursor:"pointer",fontFamily:"'Libre Franklin',sans-serif",fontSize:12}}><div style={{width:16,height:16,borderRadius:4,background:iFlandreFullOwnership?T.cyan:T.s3,border:`1px solid ${iFlandreFullOwnership?T.cyan:T.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10}}>✓</div>Pleine propriete (pas usufruit/nue-propriete)</label>
              <label onClick={()=>{setIFlandreRegister3Y(!iFlandreRegister3Y);setIRes(null);}} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,cursor:"pointer",fontFamily:"'Libre Franklin',sans-serif",fontSize:12}}><div style={{width:16,height:16,borderRadius:4,background:iFlandreRegister3Y?T.cyan:T.s3,border:`1px solid ${iFlandreRegister3Y?T.cyan:T.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10}}>✓</div>Inscription dans les 3 ans</label>
              <label onClick={()=>{setIFlandreStay1Y(!iFlandreStay1Y);setIRes(null);}} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontFamily:"'Libre Franklin',sans-serif",fontSize:12}}><div style={{width:16,height:16,borderRadius:4,background:iFlandreStay1Y?T.cyan:T.s3,border:`1px solid ${iFlandreStay1Y?T.cyan:T.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10}}>✓</div>Maintien de l'inscription >= 1 an</label>
            </div>}
            ...
            {sRes.advantageMode !== "none" && (
              <div style={{ padding: "8px 12px", background: sRes.advantageNotCalculated ? T.orange + "11" : T.green + "08", border: `1px solid ${sRes.advantageNotCalculated ? T.orange + "44" : T.green + "44"}`, borderRadius: 8, fontSize: 11, marginTop: 8 }}>
                {sRes.advantageNotCalculated ? (
                  ...
                  <>Avantage applique : {sRes.advantageMode === "full_exemption" ? "Exoneration totale (logement familial)" : sRes.advantageMode === "reduced_rate" ? "Tarif reduit (logement familial)" : "Tarif partenaire"}</>
                )}
              </div>
            )}
            <Disclaimer>L'avantage s'applique UNIQUEMENT sur la part logement familial declaree. Verifiez les conditions regionales exactes aupres d'un notaire.</Disclaimer><SourceBadge type="Source" source="notaire.be / SPF Finances" bareme="2024-2026" /><PrintBtn /></Card>)}</div>)}
