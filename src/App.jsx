...
/*  ══════════════════════════════════════════════════════════
   ...
   Patch DESIGN : Dame de Justice en fond + contrastes blancs
   ══════════════════════════════════════════════════════════  */

//  SVG Dame de Justice (Justitia) optimisé pour fond — eyes blindfolded, balance, sword
const JUSTITIA_SVG = `data:image/svg+xml;base64,`;
...
const calcRR = (p, reg, conditionsMet, sautsPEB = 0, flandreCond2026 = null, charges = 0, valeurVenale = 0) => {
  ...
  if (reg === "bruxelles") {
    ...
    if (prix <= 600000) abattementTotal += 200000;
    if (sautsPEB >= 2) abattementTotal += (sautsPEB * 25000);
...
const calcReducedHomeTax = (amount, reg) => {
  const BR_WAL = [
    { max: 25000,  r: 0.01 }, { max: 50000,  r: 0.02 }, { max: 175000, r: 0.05 },
    { max: 250000, r: 0.12 }, { max: 500000, r: 0.24 }, { max: Infinity, r: 0.30 }
  ];
  const BR_BRU = [
    { max: 50000,  r: 0.02  }, { max: 100000, r: 0.053 }, { max: 175000, r: 0.06  },
    { max: 250000, r: 0.12  }, { max: Infinity, r: 0.12  }
  ];
...
const getBelgianLegalHolidays = (year) => {
  ...
  const holidays = new Set([
    `${year}-01-01`, toIsoLocal(new Date(easterSunday.getTime() + 86400000)), `${year}-05-01`,
    toIsoLocal(new Date(easterSunday.getTime() + 39 * 86400000)), toIsoLocal(new Date(easterSunday.getTime() + 50 * 86400000)),
...
const addThreeWorkingDays = (date) => {
  ...
  let added = 0;
  while (added < 3) { result.setDate(result.getDate() + 1); if (isBelgianWorkingDay(result)) added++; }
  return result;
};
const calcNewRegime = (months, isDemission) => {
  ...
  if (isDemission) {
    ...
    if (years < 7) return 10; if (years < 8) return 12; return 13;
  } else {
    ...
    if (years < 20) return 60; if (years < 21) return 62; return 62 + (years - 20);
  }
};
const calcPreavis = (startDate, notifDate, notifType, salaryBrut, isOuvrier, isDemission) => {
  ...
  const totalMonths = Math.max(0, Math.round((ancienneteFin - start) / (1000 * 60 * 60 * 24 * 30.44)));
  ...
  if (isDemission) {
    ...
    } else { weeksNew = calcNewRegime(totalMonths, isDemission); }
  }
...
export default function JuratonePro() {
  ...
  const loadDemoData = () => { 
    ...
    const realResponse = [
      { month: "Mars 2024", value: 131.62 }, { month: "Avril 2024", value: 132.09 }, { month: "Mai 2024", value: 132.32 },
      { month: "Juin 2024", value: 132.55 }, { month: "Juillet 2024", value: 133.00 }, { month: "Aout 2024", value: 133.40 },
      { month: "Septembre 2024", value: 133.05 }, { month: "Octobre 2024", value: 133.56 }, { month: "Novembre 2024", value: 133.91 },
      { month: "Decembre 2024", value: 133.73 }, { month: "Janvier 2025", value: 135.52 }, { month: "Fevrier 2025", value: 135.79 },
      { month: "Mars 2025", value: 135.91 }, { month: "Avril 2025", value: 134.77 }, { month: "Mai 2025", value: 134.54 },
      { month: "Juin 2025", value: 135.04 }, { month: "Juillet 2025", value: 135.60 }, { month: "Aout 2025", value: 135.64 },
      { month: "Septembre 2025", value: 135.26 }, { month: "Octobre 2025", value: 135.76 }, { month: "Novembre 2025", value: 136.49 },
      { month: "Decembre 2025", value: 136.69 }, { month: "Janvier 2026", value: 137.38 },
      { month: "Fevrier 2026", value: 138.07 }, { month: "Mars 2026", value: 137.78 }
    ]; 
    setIdxData(realResponse); setIdxBaseMonth(realResponse[0].month); setIdxNewMonth(realResponse[realResponse.length - 1].month); setIdxApiState("success"); 
  };
  const doIndexation = () => { 
    ...
    if (baseValue && latestValue) { 
      ...
      let correctedAmount = rawNewAmount; let pebNote = ""; let blocked = false; let needsOfficialCalculator = false;
      if (idxBail) { 
        if (idxReg === "bruxelles") { 
          ...
          if (idxPeb === "Inconnu / Sans PEB") { correctedAmount = baseRent; pebNote = "Bruxelles : L'indexation requiert un bail enregistre ET un certificat PEB valide. Indexation bloquee."; blocked = true; } 
          else if (/[EFG]/.test(idxPeb) && bruxFactorApplies) { pebNote = "Bruxelles (bail < 14/10/2022 + PEB E/F/G) : Facteur de correction legal possible. Utilisez le calculateur officiel SPF pour le montant exact."; needsOfficialCalculator = true; } 
          else if (/[EFG]/.test(idxPeb)) { pebNote = "Bruxelles (bail >= 14/10/2022) : Indexation standard sous reserve de bail enregistre et PEB communique."; }
          else { pebNote = "Bruxelles : Indexation autorisee a 100% (sous reserve de bail enregistre et PEB communique)."; } 
        } else if (idxReg === "wallonie") { 
          if (idxPeb === "Inconnu / Sans PEB") { correctedAmount = baseRent; pebNote = "Wallonie : L'indexation requiert un bail enregistre ET un certificat PEB valide. Indexation bloquee."; blocked = true; }
          else if (/[DEFG]/.test(idxPeb)) { pebNote = "Wallonie (PEB D/E/F/G) : Regime de calcul specifique post-01/11/2023. Le montant exact doit etre verifie via le calculateur officiel SPF/Statbel."; needsOfficialCalculator = true; } 
          else { pebNote = "Wallonie : Indexation autorisee a 100% (sous reserve d'enregistrement et demande ecrite)."; } 
          ...
          if (/[DEFGI]/.test(idxPeb)) { pebNote += " Un correctiefactor peut s'appliquer. Referez-vous a IndexSearch."; needsOfficialCalculator = true; } 
        } 
      } 
      setIdxResult({ newAmount: (blocked || needsOfficialCalculator) ? null : correctedAmount, rawAmount: rawNewAmount, baseValue, latestValue, latestMonth: idxNewMonth, pebNote, blocked, needsOfficialCalculator }); 
    } 
  };
  ...
  const doSucc = () => { 
    ...
    const mappedRel = relMap[sRel]; let taxHome = calcSucc(home, sR, mappedRel); let taxOther = calcSucc(other, sR, mappedRel);
    let advantageMode = "none"; let warning = ""; let advantageNotCalculated = false;
    const applyExemptionToHome = () => { taxHome = 0; advantageMode = "full_exemption"; };
    if (sR === "flandre") {
      if (sRel === "Conjoint / Cohabitant legal") { if (sFlandreStraightLine) { warning = "Flandre : parente en ligne directe = exclusion de l'avantage logement familial."; } else { applyExemptionToHome(); } } 
      else if (sRel === "Cohabitant de fait (Flandre)") { if (!sFlandreFacto3ans) { warning = "Flandre : necessite 3 ans de cohabitation ininterrompue."; } else if (sFlandreStraightLine) { warning = "Flandre : cohabitant de fait parent en ligne directe = exclusion de l'avantage logement familial."; } else { applyExemptionToHome(); } }
    } else if (sR === "wallonie" && sRel === "Conjoint / Cohabitant legal") { if (sFamWallonie5ans) { applyExemptionToHome(); } else { warning = "Wallonie : necessite 5 ans de residence principale (condition supprimee pour deces a partir de 2028)."; } } 
    else if (sR === "wallonie" && sRel === "Ligne directe (Enfants/Parents)") { const reduced = calcReducedHomeTax(home, "wallonie"); if (reduced !== null) { taxHome = reduced; advantageMode = "reduced_rate"; } }
    else if (sR === "bruxelles" && sRel === "Ligne directe (Enfants/Parents)") { const reduced = calcReducedHomeTax(home, "bruxelles"); if (reduced !== null) { taxHome = reduced; advantageMode = "reduced_rate"; } }
      ...
      if (sRel === "Conjoint / Cohabitant legal" && !sFamBruxellesExclu) { applyExemptionToHome(); }
        ...
        if (sBruxCohab3yr) { warning = "Bruxelles : cohabitation de fait effective + menage commun au moins 3 ans. Un tarif reduit sur le logement familial est possible, mais le montant exact depend du regime legal applicable."; advantageMode = "reduced_rate"; advantageNotCalculated = true; } 
        else if (sBruxCohab1yr) { warning = "Bruxelles : cohabitation de fait effective + menage commun au moins 1 an. Tarif partenaire applicable (sans exoneration du logement familial)."; advantageMode = "partner_rate_only"; advantageNotCalculated = true; }
  ...
  return (
    <div style={{ fontFamily: "'Libre Baskerville',Georgia,serif", background: T.bg, color: T.txt, minHeight: "100vh", position: "relative" }}>
      {/* ═══════════════════════════════════════════════════════════
          🎨 FOND DESIGN : Dame de Justice + Formes en contraste blanc
          ═══════════════════════════════════════════════════════════ */}
      <style dangerouslySetInnerHTML={{ __html: `
        body { margin: 0; background: ${T.bgFlat}; }
        
        /* Dame de Justice en fond fixe — opacite tres faible pour ne pas generer la lecture */
        .justitia-bg {
          position: fixed;
          top: 0; left: 0; right: 0; bottom: 0;
          z-index: 0;
          pointer-events: none;
          background-image: url("${JUSTITIA_SVG}");
        ...
        /* Formes en contraste blanc — halo superieur gauche */
        .white-shape-1 {
          position: fixed;
          top: -200px; left: -200px;
          width: 700px; height: 700px;
          background: radial-gradient(circle at 30% 30%, rgba(255,255,255,0.95) 0%, rgba(255,255,255,0.6) 30%, transparent 70%);
          border-radius: 50%;
          z-index: 0;
          pointer-events: none;
        ...
        /* Forme blanche — ellipse inferieure droite */
        .white-shape-2 {
          position: fixed;
          bottom: -150px; right: -100px;
          width: 600px; height: 500px;
          background: radial-gradient(ellipse at 70% 60%, rgba(255,255,255,0.85) 0%, rgba(255,255,255,0.3) 50%, transparent 80%);
          border-radius: 50%;
          z-index: 0;
          pointer-events: none;
          mix-blend-mode: overlay;
        ...
        /* Forme blanche — bandeau diagonal central */
        .white-shape-3 {
          position: fixed;
          top: 30%; left: 50%;
        ...
        /* Forme blanche — petit cercle en haut a droite */
        .white-shape-4 {
          position: fixed;
        ...
        @media print {
          ...
          .no-print, .justitia-bg, .white-shape-1, .white-shape-2, .white-shape-3, .white-shape-4 { display: none!important; }
        }
      ` }} />
      
      {/* Elements de fond decoratifs */}
      <div className="justitia-bg" />
      <div className="white-shape-1" />
      <div className="white-shape-2" />
      <div className="white-shape-3" />
      <div className="white-shape-4" />
      ...
      <div className="app-content">
        ...
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 24px 60px" }}>
          ...
          {pg === "indexation" && (<div className="no-print"><SectionHead title="Indexation (Loyer & Contrats)" sub="Calcul d'indexation base sur l'Indice Sante Statbel. Donnees officielles etendues : Mars 2024 a Mars 2026. Base 2013=100." />
            ...
            {idxResult && (<Card highlight isPrintable={true}><Divider label="Rapport — Indexation" color={T.pri} /><div style={{ fontFamily: "'Libre Franklin',sans-serif", fontSize: 13, display: "flex", flexDirection: "column", gap: 8 }}><div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${T.s3}` }}><span style={{ color: T.muted }}>Montant initial</span><span style={{ fontWeight: 600 }}>{fmt(parseFloat(idxBaseAmount))}</span></div><div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${T.s3}` }}><span style={{ color: T.muted }}>Indice de base ({idxBaseMonth})</span><span style={{ fontWeight: 600 }}>{idxResult.baseValue}</span></div><div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${T.s3}` }}><span style={{ color: T.muted }}>Nouvel indice ({idxResult.latestMonth})</span><span style={{ fontWeight: 600 }}>{idxResult.latestValue}</span></div>{idxResult.blocked && <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${T.s3}` }}><span style={{ color: T.red }}>Montant brut (avant blocages PEB)</span><span style={{ fontWeight: 600, color: T.red }}><del>{fmt(idxResult.rawAmount)}</del></span></div>}<div style={{ display: "flex", justifyContent: "space-between", padding: "16px 0 4px" }}><span style={{ fontSize: 16, fontWeight: 700, color: T.pri }}>NOUVEAU MONTANT ESTIME</span>{idxResult.newAmount !== null ? (<span style={{ fontSize: 22, fontWeight: 700, color: T.pri }}>{fmt(idxResult.newAmount)}</span>) : (<span style={{ color: T.orange, fontSize: 14, fontWeight: 600 }}>Montant non calcule (calculateur officiel requis)</span>)}</div>{idxResult.needsOfficialCalculator && (<div style={{ fontSize: 11, color: T.muted, marginTop: 4 }}>Montant theorique (sans correctif legal) : {fmt(idxResult.rawAmount)}</div>)}</div>{idxBail && (<div style={{ padding: "12px 16px", background: idxResult.blocked ? T.red + "08" : idxResult.needsOfficialCalculator ? T.orange + "08" : T.green + "08", border: `1px solid ${idxResult.blocked ? T.red + "44" : idxResult.needsOfficialCalculator ? T.orange + "44" : T.green + "44"}`, borderRadius: 8, marginTop: 16, fontFamily: "'Libre Franklin',sans-serif", fontSize: 11, color: T.txt }}><strong style={{ color: idxResult.blocked ? T.red : idxResult.needsOfficialCalculator ? T.orange : T.green }}>{idxResult.blocked ? "Indexation bloquee" : idxResult.needsOfficialCalculator ? "Calcul specifique requis" : "Regles d'indexation regionales"} :</strong>{idxResult.pebNote && <p style={{ margin: "6px 0 8px", color: idxResult.blocked ? T.red : idxResult.needsOfficialCalculator ? T.orange : T.pri, fontWeight: "bold" }}>{idxResult.pebNote}</p>}<ul style={{ margin: 0, paddingLeft: 20, color: T.muted, lineHeight: 1.6 }}><li>L'indexation suppose un bail enregistre.</li><li>L'effet retroactif est limite aux 3 mois precedant la demande ecrite.</li></ul>{idxResult.blocked && <div style={{marginTop:8,padding:"6px 10px",background:T.red+"11",borderRadius:6,fontSize:10,color:T.red}}>Sans certificat PEB valide, l'indexation est juridiquement impossible dans cette region.</div>}{idxResult.needsOfficialCalculator && <div style={{marginTop:8,padding:"6px 10px",background:T.orange+"11",borderRadius:6,fontSize:10,color:T.orange}}>Pour ce cas, privilegiez le calculateur officiel SPF/Statbel pour un montant opposable.</div>}</div>)}<Disclaimer>Donnees Statbel officielles (API JSON). Pour la Wallonie et Bruxelles (PEB defavorables), privilegiez le calculateur officiel SPF/Statbel. Depuis janvier 2026, la base officielle est 2025=100 ; les valeurs 2026 sont converties ici en base 2013 via le coefficient 0,7376.</Disclaimer><SourceBadge type="Source" source="Statbel API (bestat.statbel.fgov.be)" bareme="Base 2013 - Mars 2024 a Mars 2026" /><PrintBtn /></Card>)}
          </div>)}
          ...
          {pg === "notariat" && (<div><SectionHead title="Outils notariaux" sub="Calculateurs et dossiers notaries." />
            ...
            {sRes && (<Card highlight isPrintable={true}><Divider label={"Rapport — Succession " + rl[sRes.sR]} color={T.pri} />
              ...
              {sRes.advantageMode !== "none" && (<div style={{ padding: "8px 12px", background: sRes.advantageNotCalculated ? T.orange + "11" : T.green + "08", border: `1px solid ${sRes.advantageNotCalculated ? T.orange + "44" : T.green + "44"}`, borderRadius: 8, fontSize: 11, marginTop: 8 }}>{sRes.advantageNotCalculated ? (<><Avantage legal potentiel detecte (<b>{sRes.advantageMode}</b>), mais <b>non chiffre</b> par ce simulateur.</>) : (<><Avantage applique : {sRes.advantageMode === "full_exemption" ? "Exoneration totale (logement familial)" : sRes.advantageMode === "reduced_rate" ? "Tarif reduit progressif (logement familial)" : "Tarif partenaire"}</>)}</div>)}
          ...
          {pg === "juridometre" && (<div><SectionHead title="Diagnostic de vigilance" sub="Profil de risque et actions recommandees." />
            ...
            </div></Card>) : (<div className="print-section">
